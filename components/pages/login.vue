    ...
    <div tabindex="-1" role="group" style="outline: none;">
        <div class="c--auth-box">
            ...
            <div class="auth--body" dir="ltr">
                <div class="auth--logo-box"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 240 78"
                        class="auth--logo" lang="fa">
                        <path
                    ...
                    <p class="ac--text text--align__center text--weight__regular text--size__regular logo-box--slang"
                        dir="auto" auto-dir="auto">تجربه&zwnj;ی لذت&zwnj;بخش از آموزش و یادگیری</p>
                </div>
                <p class="ac--text text--align__center text--weight__regular text--size__normal sign-in--hint"
                    dir="auto" auto-dir="auto">{{ login_msg }}</p>
                <div class="sign-in--phone-box">
                    ...
                    <v-otp-input reverse v-model="code" v-if="action == 1 && stage == 1" length="4" type="number">
                    </v-otp-input>
                    <div v-if="action == 1 && stage == 1" class="text-center mt-2 code-resend" @click="resendCode">
                        {{ code_resend }}</div>


                    <v-text-field reverse class="centered-input " v-if="stage == 0" filled rounded solo dense
                        v-model="phone" :rules="[() => !!phone || 'این فیلد الزامی است',
                        () => !!validPhone || 'شماره موبایل صحیح نمی باشد'
                        ]" required label="09123456789" prepend-inner-icon="mdi-phone"></v-text-field>

                    <v-text-field reverse v-if="action == 1 && stage == 2" filled rounded solo dense v-model="name"
                        :rules="[() => !!name || 'این فیلد الزامی است',
                        
                        ]" required label="نام شما" class="centered-input" prepend-inner-icon="mdi-account">
                    </v-text-field>

                    <v-text-field reverse v-if="action == 1 && stage == 2" class="centered-input" filled rounded solo
                        dense v-model="password" name="password" :rules="[() => !!password || 'این فیلد الزامی است',
                        () => !!validPassword || 'رمز عبور حداقل 6 رقمی باشد'
                        ]" :type="show1 ? 'text' : 'password'" @click:append="show1 = !show1" required
                        label="کلمه عبور" prepend-inner-icon="mdi-cursor-text"></v-text-field>

                    <v-text-field reverse v-if="action == 1 && stage == 2" class="centered-input" filled rounded solo
                        dense v-model="confirm_password" name="confirm_password"  :rules="[() => !!(confirm_password == password) || 'رمز عبور با تکرار آن مطابقت ندارد',
                        
                        ]" :type="show2 ? 'text' : 'password'" @click:append="show2 = !show2" required
                        label="تکرار کلمه عبور" prepend-inner-icon="mdi-cursor-text"></v-text-field>



                </div>
                <p v-if="stage == 0"
                    class="ac--text text--align__center text--weight__regular text--size__regular sign-in--privacy-policy"
                    dir="auto" auto-dir="auto">با ورود و یا ثبت نام در وبلایت شما <a target="_blank"
                        href="https://weblite.im/privacy-policy" rel="noopener noreferrer"
                        style="font-weight: bold; color: rgb(0, 192, 255);">قوانین حریم خصوصی</a> آن را
                    می&zwnj;پذیرید.
                </p>
...
export default {
    ...
    methods: {
        ...
        async resendCode() {
            ...
            try {
                var resend = await this.$axios.post("register/resend-code", { phone: this.phone }).then(res => res.data);
                ...
                if (resend.success == 0) {
                    ...
                    var timer = setInterval(() => {
                        this.remain--;
                        this.code_resend = "ارسال مجدد کد در 00:" + (this.remain < 10 ? `0${this.remain}` : this.remain)
                        if (this.remain == 0) {
                            clearInterval(timer);
        ...
        async signIn() {
            ...
            if (this.stage == 0) {
                ...
                try {
                    var check = await this.$axios.get("login/check?phone=" + this.phone).then(res => res.data);
            ...
            //register => to verify code
                ...
                try {
                    var verify = await this.$axios.post("register/verify-code", { phone: this.phone, code: this.code }).then(res => res.data);
            ...
            //register => finish
                ...
                try {
                    var finish = await this.$axios.post("register/finish", { phone: this.phone, code: this.code, name: this.name, password: this.password, confirm_password: this.confirm_password }).then(res => res.data);
    ...
    computed: {
        ...
        validFinal() {
            if (this.password.length == this.confirm_password.length && this.name && this.validPassword) {
                return true;
            }
            return false;
        },
